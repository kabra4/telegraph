...
export default class ReminderController {
    ...
    protected async processBeforehand(ctx: actionCtx, beforehand: boolean) {
        const user = await TgUser.getTgUserByTgId(ctx.callbackQuery.from.id);
    ...
    private async processBeforehandTimeButtonInput(
        ...
        if (option === "custom") {
            ...
            user.save().then(() => {
                ls.setLocale(user.data.language);
                ctx.reply(
                    ls.__("reminder.questions.custom_beforehand_time.text")
                );
            });
        } else {
    ...
    protected async processTimeInput(
        ...
        const user = await TgUser.getTgUserByTgId(ctx.from.id);
        ...
        user.save().then(() => {
            ...
            ctx.reply(
                ...
                Markup.inlineKeyboard([
                    Markup.button.callback(
                        ls.__(
                            "reminder.questions.has_it_beforehand.options.yes"
                        ),
                        "rmdr.beforehand.yes"
                    ),
                    Markup.button.callback(
                        ls.__(
                            "reminder.questions.has_it_beforehand.options.no"
                        ),
                        "rmdr.beforehand.no"
                    ),
                ])
    ...
    public async Start(
        ...
        if (ctx.message.text == "/rmdr") {
            const user = await TgUser.getTgUserByTgId(ctx.message.from.id);
    ...
    private async processRepeatNo(ctx: actionCtx) {
        ...
        user.data.reminder_options.repeat_is_checked = true;
        user.data.is_currently_doing = "reminder.pattern.date";
        user.save();
        ...
        ctx.editMessageText(
            ...
            await CalendarMaker.makeMonthsGrid(locale_, current_year, true)
        );
    }
    ...
    public async processCalendarNavigation(ctx: actionCtx, navigation: string) {
        ...
        if (navigation === "prev") {
            ...
            ls.setLocale(user.data.language);
            ctx.replyWithMarkdownV2(
                ls.__("reminder.questions.send_time.single")
            );
        }
    }
    ...
    public async processMonthCalendar(ctx: actionCtx) {
        ...
        const user = await TgUser.getTgUserByTgId(ctx.callbackQuery.from.id);
        const locale_date = user.data.language;
    ...
    public async processWeekDayInput(ctx: actionCtx) {
        ...
        const user = await TgUser.getTgUserByTgId(userId);
        ...
        const updatedMarkup = await await CalendarMaker.weekdaysMarkup(
            user.data.language,
            user.data.reminder_options.checked_days
        );
    ...
    public async processMessage(
        ...
        const user = await TgUser.getTgUserByTgId(ctx.message.from.id);
        if (user.data.is_currently_doing !== "reminder.name") {
            return;
        }
        user.data.reminder_options.time = ctx.message.text;
        user.data.is_currently_doing = "none";
        user.save();
        ls.setLocale(user.data.language);
